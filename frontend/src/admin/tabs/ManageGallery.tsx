...
interface ManageGalleryProps {
  ...
  adminEditMediaType: string;
  setAdminEditMediaType: (val: string) => void;
...
export default function ManageGallery({
  ...
  return (
    <>
      {editModalOpen && editModalType === 'gallery' ? (
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            <div>
              <h2 className="text-base font-extrabold text-[#002147]">
                {activeEditItem ? 'Edit Media Galeri' : 'Tambah Media Galeri Baru'}
              </h2>
              <span className="text-[11px] text-slate-400 font-medium block">
                Isi detail data dokumentasi media PPID dengan lengkap.
              </span>
            </div>
            <button
              type="button"
          ...
          <form onSubmit={handleSaveCrudItem} className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-600 uppercase tracking-wider block">
                Judul Media Galeri <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                required
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-600 uppercase tracking-wider block">Tipe Media</label>
                <select
                  value={adminEditMediaType}
                  onChange={(e) => setAdminEditMediaType(e.target.value)}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-600 uppercase tracking-wider block">Keterangan Tambahan</label>
              <input
                type="text"
                value={adminEditDescription}
                onChange={(e) => setAdminEditDescription(e.target.value)}
                className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all font-medium text-slate-805"
              />
            </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-extrabold text-[#002147]">Daftar Media Galeri</h2>
              <span className="text-[11px] text-slate-400 font-medium block">
                Unggah foto dokumentasi kegiatan layanan transparansi informasi kampus.
              </span>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 gap-3">
            {adminGalleries.length === 0 ? (
              <div className="py-12 text-center text-slate-400 text-xs font-medium">
                Belum ada media galeri terdaftar.
              </div>
            ) : (
              adminGalleries.map((gal) => (
                <div
                  key={gal.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <img
                      src={resolveImageUrl(gal.media_url)}
                      alt={gal.title}
                      className="h-14 w-20 rounded-lg object-cover shrink-0 border border-slate-100"
                    />
                    <div className="min-w-0 text-left">
                      <span className="text-[10px] text-slate-400 font-medium uppercase block">{gal.media_type}</span>
                      <h4 className="text-sm font-bold text-slate-800 truncate mt-0.5" title={gal.title}>
                        {gal.title}
                      </h4>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:border-l sm:border-slate-200 sm:pl-3 shrink-0">
                    ...
                    <button
                      onClick={() => handleDeleteCrudItem('gallery', gal.id)}
