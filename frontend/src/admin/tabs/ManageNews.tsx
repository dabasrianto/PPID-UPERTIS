      ...
      {editModalOpen && editModalType === 'post' ? (
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            <div>
              <h2 className="text-base font-extrabold text-[#002147]">
                {activeEditItem ? 'Edit Rilis Berita KIP' : 'Tulis Berita KIP Baru'}
              </h2>
              <span className="text-[11px] text-slate-400 font-medium block">
                Tulis detail data berita/informasi terkait Keterbukaan Informasi Publik.
              </span>
            </div>
            <button
              type="button"
          ...
          <form onSubmit={handleSaveCrudItem} className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-600 uppercase tracking-wider block">
                Judul Berkas / Rilis Berita <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                required
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-600 uppercase tracking-wider block">Ringkasan Excerpt</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-655 uppercase tracking-wider block">Konten Berita (HTML/Markdown)</label>
              <textarea
                required
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-extrabold text-[#002147]">Kelola Rilis Berita KIP</h2>
              <span className="text-[11px] text-slate-400 font-medium block">
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 text-left">
                      ...
                      <h4 className="text-sm font-bold text-slate-800 truncate mt-0.5" title={post.title}>
                        {post.title}
                      </h4>
                      {post.excerpt && (
                        <p className="text-[11px] text-slate-400 font-medium truncate">{post.excerpt}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:border-l sm:border-slate-200 sm:pl-3 shrink-0">
                    ...
                    <button
                      onClick={() => handleDeleteCrudItem('posts', post.id)}
